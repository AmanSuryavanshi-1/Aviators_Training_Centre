...
import { Save, Eye, X, Plus, Loader2, FileText, Settings, AlertCircle } from 'lucide-react';
import { toast } from 'sonner';
import { Alert, AlertDescription } from '@/components/ui/alert';
import { ValidationSummary } from './ValidationSummary';

interface SimplifiedBlogFormData {
  title: string;
  slug: string;
  excerpt: string;
  content: string;
...
const defaultFormData: SimplifiedBlogFormData = {
  title: '',
  slug: '',
  excerpt: '',
  content: '',
...
interface SimplifiedBlogEditorProps {
  initialData?: Partial<SimplifiedBlogFormData>;
  onSave?: (data: SimplifiedBlogFormData) => Promise<void>;
  onPreview?: (data: SimplifiedBlogFormData) => void;
  loading?: boolean;
  mode?: 'create' | 'edit';
}

export function SimplifiedBlogEditor({ 
  initialData, 
  onSave, 
  onPreview, 
  loading = false, 
  mode = 'create' 
}: SimplifiedBlogEditorProps) {
  // Initialize form data with proper defaults to prevent uncontrolled input warnings
  const [formData, setFormData] = useState<SimplifiedBlogFormData>(() => {
    if (initialData) {
      return {
        ...defaultFormData,
        ...initialData,
  ...
  const handleInputChange = useCallback((field: keyof SimplifiedBlogFormData, value: any) => {
    setFormData(prev => ({
      ...prev,
      [field]: value,
    }));
              ...
              <CardContent className="max-h-[80vh] overflow-y-auto">
                <div className="space-y-4">
                    ...
                    <div className="border rounded-lg p-4 bg-yellow-50">
                      ...
                      <div className="mt-3 pt-3 border-t border-yellow-200">
                        <div className="flex items-center justify-between text-xs">
                          <span className="text-yellow-700 font-medium">SEO Score:</span>
                          <div className="flex items-center gap-2">
                            <div className="w-16 h-2 bg-yellow-200 rounded-full overflow-hidden">
                              <div 
                                ...
                                style={{ 
                                  width: `${Math.min(100, (
                                    (formData.seoTitle ? 25 : 0) +
                                    (formData.seoDescription ? 25 : 0) +
                                    (formData.focusKeyword ? 25 : 0) +
                                    (formData.content && formData.content.length > 300 ? 25 : 0)
                                  ))}%` 
                                }}
                              />
                            </div>
                  ...
                  <div className="border rounded-lg p-4 bg-slate-50">
                    <h3 className="text-sm font-semibold text-slate-700 mb-3">Content Statistics</h3>
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        <span className="text-slate-600">Word Count:</span>
                        <span className="ml-2 font-medium text-slate-900">{wordCount}</span>
                      </div>
                      <div>
                        <span className="text-slate-600">Reading Time:</span>
                        <span className="ml-2 font-medium text-slate-900">{readingTime} min</span>
                      </div>
                      <div>
                        <span className="text-slate-600">Characters:</span>
                        <span className="ml-2 font-medium text-slate-900">{formData.content.length}</span>
                      </div>
                      <div>
                        <span className="text-slate-600">Tags:</span>
                        <span className="ml-2 font-medium text-slate-900">{formData.tags.length}</span>
                      </div>
                    </div>
                  </div>
                </div>
