...
async function fixMarkdown() {
    try {
        ...
        const result = await cloudinary.api.resources({
            ...
            max_results: 500, // Ensure we get all of them
        });
        ...
        for (const resource of result.resources) {
            ...
            const filenameWithUnderscores = publicIdParts[publicIdParts.length - 1]; // ASSET-13_Homepage_Screenshot

            const match = filenameWithUnderscores.match(/^(ASSET-\d+)/);
            if (match) {
                const assetPrefix = match[1]; // e.g., "ASSET-13"
                assetMap.set(assetPrefix, resource.secure_url);
                console.log(`Mapped ${assetPrefix} -> ${resource.secure_url}`);
            } else {
        ...
        for (const docFile of MARKDOWN_FILES) {
            ...
            for (const [key, correctUrl] of assetMap.entries()) {
